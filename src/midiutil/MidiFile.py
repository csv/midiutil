...
class MIDIFile:
    ...
    def addChord(self, track, channel, root, chord, time, duration, volume):
        """
        Add notes to the MIDIFile object
        ...
        Arguments:
            ...
            volume: the volume (velocity) of the note. [Integer, 0-127].
        """
        chord_pattern = chord_to_midi(chord)
        root = note_to_midi(root)
        chord_notes = [root + note for note in chord_pattern if (root + note) < 127]
    ...
    def addSysEx(self,track, time, manID, payload):
        ...
        """
        self.tracks[track].addSysEx(time, dmanID, payload)
